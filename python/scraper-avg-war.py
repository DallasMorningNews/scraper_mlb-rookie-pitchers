...
def find_exclusives(pitcher):
    ...
    if len(mlb_rows) > 0 and 'Pitcher' in graph_text[1] and pitcher["team"] in draft_text[len(draft_text) - 2]:
        ...
        if pitcher["short_team"] in rookie_cells[1].text:
            ...
            pitcher["seasons"] = seasons
            pitcher["war"] = round(war, 1)
            pitcher["war/season"] = round((round(war, 1) / seasons), 2)
...
# teams=["TEX", "ANA", "HOU", "OAK", "TOR", "ATL", "MIL", "STL", "CHC", "TBD", "ARI", "LAD", "SFG"]
# teams=["CLE", "SEA", "FLA", "NYM", "WSN", "BAL", "SDP", "PHI", "PIT", "BOS", "CIN", "COL"]
# teams=["KCR", "DET", "MIN", "CHW", "NYY",]
# # teams=["MIN"]
...
for team in teams:
    ...
    with open('data/exclusives-avg-war.csv', 'a') as output_file:
